...
export default function InputScreen({ navigation }: Props) {
  ...
  const [placeIdx, setPlaceIdx] = useState(() => BIRTH_PLACE_PRESETS.findIndex((p) => p.label === '東京'));
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ padding: spacing(2), paddingBottom: spacing(6) }}>
      ...
      <Text style={styles.helperText}>
        リストにない場合は、一番近い都市を選べば大丈夫です。ここでの選択はアセンダントの計算だけに使われるので、他の占術の結果には影響しません。
      </Text>
...
const styles = StyleSheet.create({
  ...
  helperText: { color: colors.textMuted, fontSize: 12, marginBottom: spacing(1), lineHeight: 17 },
  input: {
    backgroundColor: colors.surface, color: colors.text, borderRadius: 14,
    paddingHorizontal: spacing(1.5), paddingVertical: spacing(1.2), borderWidth: 1.5, borderColor: colors.border,
  },
  ...
  chip: {
    ...
    backgroundColor: colors.surface, borderWidth: 1.5, borderColor: colors.border, marginRight: spacing(1), marginBottom: spacing(1),
  },
  chipActive: { backgroundColor: colors.primary, borderColor: colors.primary },
  chipText: { color: colors.textMuted },
  chipTextActive: { color: colors.onPrimary, fontWeight: '700' },
  submitButton: {
    marginTop: spacing(4), backgroundColor: colors.primary, borderRadius: 20,
    paddingVertical: spacing(1.8), alignItems: 'center', ...cardShadow,
  },
